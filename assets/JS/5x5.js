...
sizes.innerHTML = "<div><p class='frame-size'>Frame size:</p><p class='frame-size'>5x5</p></div>"
...
function animate5x5(){
    ...
    function orderChange(item1, item2){
       ...
       }else if([...gameFrame.childNodes].indexOf(item2) === [...gameFrame.childNodes].indexOf(item1) + 5){
        gameFrame.replaceChild(item1, item2)
        gameFrame.insertBefore(item2, item1.previousSibling.previousSibling.previousSibling.previousSibling)
       }else if([...gameFrame.childNodes].indexOf(item2) === [...gameFrame.children].indexOf(item1) - 5){
        gameFrame.replaceChild(item1, item2)
        gameFrame.insertBefore(item2, item1.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling)
       }  
       curtain.style.display = 'none';
       combinationChecker(5)
    }
    ...
    gameFrame.childNodes.forEach((fragment, index)=>{
        fragment.addEventListener('click', function eventListener() {
            ...
            }else if(fragment.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling && fragment.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling.hasAttribute('id')){
                fragment.style.transition = 'all 0.4s ease-out';
                fragment.style.transform = 'translateY(105%)'
                curtain.style.display = 'block'
                setTimeout(()=>{
                    fragment.style.transform = 'translateY(0%)'
                    orderChange(fragment, fragment.nextSibling.nextSibling.nextSibling.nextSibling.nextSibling)
                    fragment.addEventListener('click', eventListener)  
                }, 1000)

            }else if(fragment.previousSibling.previousSibling.previousSibling.previousSibling.previousSibling && fragment.previousSibling.previousSibling.previousSibling.previousSibling.previousSibling.hasAttribute('id')){
                fragment.style.transition = 'all 0.4s ease-out';
                fragment.style.transform = 'translateY(-105%)'
                curtain.style.display = 'block'
                setTimeout(()=>{
                    fragment.style.transition = 'none'
                    fragment.style.transform = 'translateY(0%)'
                    orderChange(fragment, fragment.previousSibling.previousSibling.previousSibling.previousSibling.previousSibling)
                    fragment.addEventListener('click', eventListener)  
                }, 1000)
...
function puzzle5x5(){
    ...
    fragment18, fragment19, fragment20, fragment21, fragment22, fragment23, fragment24, fragment25];
    let min = 0;
